...
def render_cuda(
    ...
    projection_matrix = get_projection_matrix(near, far, fov_x, fov_y).type_as(extrinsics)
    projection_matrix = rearrange(projection_matrix, "b i j -> b j i")
    view_matrix = rearrange(extrinsics.float().inverse().type_as(extrinsics), "b i j -> b j i")
    ...
    for i in range(b):
        ...
        all_images.append(image.type_as(gaussian_means))
        all_radii.append(radii.type_as(gaussian_means))
...
def render_depth_cuda(
    ...
    camera_space_gaussians = einsum(
        extrinsics.float().inverse().type_as(gaussian_means), homogenize_points(gaussian_means), "b i j, b g j -> b g i"
    )
